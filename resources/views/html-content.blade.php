...
<head>
    ...
    <style type="text/css">
        ...
        .page {
            ...
            padding: 10mm; /* Margin 1cm semua sisi */
        ...
        tr[bgcolor="#E1E1E1"] td {
            ...
            font-weight: bold;
            text-align: center;
            white-space: nowrap; /* Tambahkan agar tidak wrap */
            overflow: hidden;
        }
        ...
        .content-container {
            ...
            margin: 0 auto;
            position: relative;
            min-height: 277mm; /* Tinggi kontainer sesuai page */
        }
...
<body id='faktur'>
    <div class="mypg">
        @php
            ...
            // Logika untuk membagi data ke dalam halaman
            $all_data = [];
            $max_rows_per_page = 25; // Jumlah maksimum baris per halaman
                ...
                foreach($kimia as $item) {
                    if($item && !empty($item->analysis)) {
                        $all_data[] = [
                            ...
                            'hasil_pengujian' => $item->hasil_pengujian ?? '',
                            'rujukan' => $item->dataPemeriksaan->rujukan ?? '-',
                            'satuan' => $item->dataPemeriksaan->satuan_hasil_pengujian ?? '-',
                            'keterangan' => $item->keterangan ?? '-'
                        ];
                    }
                }
            ...
            // 3. HASIL LAIN - GUNAKAN URUTAN YANG SAMA DENGAN DETAIL
            ...
            foreach($all_data as $item) {
                if($item['type'] == 'header') {
                    // Jika header tidak muat di halaman saat ini, buat halaman baru
                    if($current_row_count >= $max_rows_per_page && !empty($current_page)) {
                        $pages[] = $current_page;
                        $current_page = [];
                        $current_row_count = 0;
                    }
                    ...
                    // Jika baris data, hitung apakah masih muat
            ...
            <div class="content-container">
                ...
                <table style='border-collapse:collapse; border:none;' border='0' width='100%'>
                    ...
                    @foreach($page_data as $item)
                        @if($item['type'] == 'header')
                            <tr>
                                <td colspan="5" style="padding-left: 15px; white-space: nowrap;"><b>{{ $item['title'] }}</b></td>
                            </tr>
                            @php $baris = 1; @endphp
